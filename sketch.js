...
//b.setCollider("circle",0,0,25);
  ...
  flo=createSprite(200,310,400,10);
  //flo.shapeColor=220;
  flo.visible=false;
  floor=createSprite(200,300,800,10);
  floor.addAnimation("fl",floor1);
   floor.velocityX=-3;
  cactusgroup=new Group();
  cloudgroup=new Group();
  restartgame=createSprite(240,160,10,10);
...
function draw() {
  ...
  if (gamestate==1){
    ...
    if (keyDown(UP_ARROW)&& b.y>200){
    b.velocityY=-10;
    jumpsound.play();
  }//end of if statement(trexmove)
     if (frameCount%50==0){
  cloud=createSprite(400,150,20,20);
  cloud.addAnimation("cloud",cloud1);
  cloud.scale=0.5;
  cloud.velocityX=-3;
    cloud.y=random(150,200);
  cloud.x=random(400,500);
    cloud.depth=0.21;
  ...
  if (frameCount%70==0){
    ...
    obstacle1=createSprite(610,290,10,10);
    obstacle1.scale=0.6;
...
if (score>highscore){
  ...
  text("HighScore:",300,40);
  text(scorezeros()+highscore,365,40);
  text("Score:",300,20);
  text(scorezeros()+score,350,20  );
  // abcd=scorezeros();
